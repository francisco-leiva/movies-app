...
export default async function SearchPage({ searchParams }) {
  ...
  const { searchResults, totalPages } = await searchShows(q, page)
    ...
    <main className='pt-24 pb-4'>
      <section className='max-w-4xl min-h-[calc(100vh-11rem)] w-full px-2 grid grid-cols-cards justify-items-center gap-x-4 gap-y-4 sm:px-0 md:mx-auto'>
        {searchResults.length ? (
          searchResults.map(({ id, title, posterPath, type }, index) => {
            if (type === 'movie') {
              return (
                <MovieCards
                  key={id}
      ...
      {searchResults.length ? (
        <CustomPagination path={'search'} query={q} totalPages={totalPages} />
      ) : null}
    </main>
  )
}
